...
var VideoListView = Class.create({
	initialize : function( id){
		...
		if(screen.width == 480)
			...
			this.row_count = 2;
		this.list.style.height = this.item_height + "px";
		var self = this;
		this.tap = this.tap;
	},
	insert : function(_obj){
		if(_obj instanceof Array){
			...
			if(this.size > this.row_count ){
				this.size = 1;
				this.list.style.height = this.list.offsetHeight + this.item_height + "px";
			}
...
var ChannelListView = Class.create({
	...
	tap : function(_parent){
		...
		item.style.background="url(images/channel_tap_bg.png) repeat-y bottom right";
